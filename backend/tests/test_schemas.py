...
def test_product_card_contains_android_stable_fields():
    ...
    assert {key: dumped[key] for key in (
        "product_id",
        "title",
        "brand",
        "price",
    ...
    assert dumped["price_range"] == ""
    assert dumped["rating"] == 0.0
    assert dumped["sold_count"] == 0
    assert dumped["review_count"] == 0
    assert dumped["marketing_desc"] == ""
